...
You can train baseline models on Mr.Sum dataset from the scratch using the commands below.
...
- [Download PGL-SUM checkpoint]()
- [Download VASNet checkpoint]()
- [Download SL-module checkpoint]()

** We will further release more checkpoints upon acceptance of the paper.

Follow the command below to inference on trained checkpoints.
```
python main.py --train False --model <model_type> --ckpt_path <checkpoint file path> --tag inference
```
For example, if you download the VASNet checkpoint and place it inside the `dataset` folder, you can use the command below.
```
...
We provide the [`SimpleMLP`](networks/mlp.py) summarization model as an toy example.
...
This code referred the codes from [PGL-SUM](https://github.com/e-apostolidis/PGL-SUM), [VASNet](https://github.com/ok1zjf/VASNet), and [SL-module](https://github.com/ChrisAllenMing/Cross_Category_Video_Highlight).
